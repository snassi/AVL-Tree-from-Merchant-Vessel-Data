...
//THIS IS THE NEWLY WRITTEN AVLTREE

/* // template definitions of my new tree

//THIS METHOD IS NEEDED IN ORDER FOR THIS TO WORK
//THE MAX FUNCTION
template<class DT> const DT& max(const DT& a, const DT& b) {
	return (a<b) ? b : a;
}
...
{
public:
	HashNode<DT1,DT2>* hash;
	string name;
	DT1 data;
	avl_node<DT1,DT2>* left; //left subtree
	avl_node<DT1,DT2>* right; //right subtree
};
...
private:
	avl_node<DT1,DT2>* root;
public:
	int height(avl_node<DT1,DT2>* a);
	int diff(avl_node<DT1,DT2>* a);
	avl_node<DT1,DT2>* rr_rotation(avl_node<DT1,DT2>* a);	//right-rotation
	avl_node<DT1,DT2>* ll_rotation(avl_node<DT1,DT2>* a);	//left-rotation
	avl_node<DT1,DT2>* lr_rotation(avl_node<DT1,DT2>* a);	//left-right rotation (double left)
	avl_node<DT1,DT2>* rl_rotation(avl_node<DT1,DT2>* a);	//right-left rotation (double right)
	avl_node<DT1,DT2>* balance(avl_node<DT1,DT2>* a);
	avl_node<DT1,DT2>* insert(avl_node<DT1,DT2>* a, int b, HashNode<DT1,DT2>* HN);
	void display(avl_node<DT1,DT2>* a, int b);
	void inorder(avl_node<DT1,DT2>* a);
	void preorder(avl_node<DT1,DT2>* a);
	void postorder(avl_node<DT1,DT2>* a);
	avl_tree();
};

template<class DT1, class DT2>
avl_tree<DT1,DT2>::avl_tree()	{
	root = NULL;
}

template<class DT1, class DT2>
int avl_tree<DT1,DT2>::height(avl_node<DT1,DT2>* a)	{
	int h = 0;
	if(a != NULL)
	{
	int l_height = height(a->left);		//left height is height of left-subtree
	int r_height = height(a->right);	//right height is height of right-subtree
	int max_height = max(l_height, r_height);	//max of the two subtrees is height of tree
	h = max_height + 1;
	}
	return h;
}
...
template<class DT1, class DT2>
int avl_tree<DT1,DT2>::diff(avl_node<DT1,DT2>* a)
{
	// from this we can determine that the difference is in fact the left height minus the right
	// then we return the balance factor
...
template<class DT1, class DT2>
avl_node<DT1,DT2>* avl_tree<DT1,DT2>::rr_rotation(avl_node<DT1,DT2>* a)
{
	// this is the right rotation
	// this is called in the left-right rotation
	avl_node<DT1,DT2>* temp;
	// temp is set equal to the right subtree of a
	temp = a->right;
	// the right subtree of a is set equal to the left subtree of temp (or the left subtree of the right subtree of a)
	a->right = temp->left;
	// the left subtree of temp is set to a
	temp->left = a;
	// new tree returned
	return temp;
}
...
template<class DT1, class DT2>
avl_node<DT1,DT2>* avl_tree<DT1,DT2>::ll_rotation(avl_node<DT1,DT2>* a)
{
	// this is the left rotation
	avl_node<DT1,DT2>* temp;
	//temp is set equal to the left subtree of a
	temp = a->left;
	// left subtree of a is set equal to the right subtree of temp (or the right subtree of the left subtree of a)
	a->left = temp->right;
	// the right subtree of temp is set to a
	temp->right = a;
	// new tree returned
	return temp;
}
...
template<class DT1, class DT2>
avl_node<DT1,DT2>* avl_tree<DT1,DT2>::lr_rotation(avl_node<DT1,DT2>* a)
{
	// this is called the left-right rotation
	avl_node<DT1,DT2>* temp;
	// temp is set equal to the left subtree of a
	temp = a->left;
	// the left subtree of a is set equal to the right rotation of temp (left subtree of a, "itself")
	a->left = rr_rotation(temp);
	// then the left rotation of the right rotation of the left subtree is returned
	// the new tree essentially
	return ll_rotation(a);
}

template<class DT1, class DT2>
avl_node<DT1,DT2>* avl_tree<DT1,DT2>::rl_rotation(avl_node<DT1,DT2>* a)
{
	// this is called the right-left rotation
	avl_node<DT1,DT2>* temp;
	// temp is then set equal to the right subtree of a
	temp = a->right;
	// the right subtree of a is set equal to the left rotation of the right subtree of a
	a->right = ll_rotation(temp);
	// then the right rotation of the left rotation of the right subtree is retured
	// the new tree
	return rr_rotation(a);
}

template<class DT1, class DT2>
avl_node<DT1,DT2>* avl_tree<DT1,DT2>::balance(avl_node<DT1,DT2>* a)
{
	// balance factor is declared and set equal to the difference of a
	int bal_factor = diff(a);
	//large if else statement to see how it is balanced
	if(bal_factor > 1)
	{
		if(diff(a->left) > 0)
		// if the difference of the left subtree is greater that 0
		// a is set equal to the left rotation of a
		a = ll_rotation(a)'
		else
		// or else it is set to the left-right rotation of itself
		a = lr_rotation(a);
	}
	// else if the balance factor is less that -1
	else if(bal_factor < -1)
	{
		// if the difference of the right subtree of a is greater than 0
		if(diff(a->right) > 0)
		// a is set to the right-left rotation of itself
			a = rl_rotation(a);
		// or else it is set to the right rotation of itself
		else
			a = rr_rotation(a);
	}
		// return the new tree
...
template<class DT1, class DT2>
avl_node<DT1,DT2>* avl_tree<DT1,DT2>::insert(avl_node<DT1,DT2>* a, int b, Hashnode<DT1,DT2>* HN)
{
	// a is the root
	// b is the value of the node
	// HN is the hashnode of the node
	
	//check if the root is null
	if(a==NULL)
	{
		// make a new node
		a = new avl_node<DT1,DT2>();
		// enter in all the data coming in from parameters
		a->hash = HN;
		a->data = b;
		a->left = NULL;
		a->right = NULL;
	...
	// else b is less that the data from a (when it is not null)
	else if(b < a->data)
	{
	// set the left subtree of a to the insertion of the left subtree of a, b, and the hashnode of a
		a->left = insert(a->left, b, a->hash);
		// recursively call the function
		a = balance(a);
	}
	// else if b is less than or equal to the data present in a (when it is not null)
	else if(b >= a->data)
	{
	// set the right subtree of a to the insertion of the right subtree of a, b, and the hashnode of a
		a->right = insert(a->right, b, a->hash);
	// recursively call the function
		a = balance(a);
	}
	// after all is said and done return the tree
	return a;
}
...
void avl_tree<DT1,DT2>::display(avl_node<DT1,DT2>* a, int b)
{
	...
	// check if a is the root
		if(a == root)
		cout << "Root -> ";
	// loop until i = b and while a not equal to the root
		for(i = 0; i < b && a != root; i++)
		cout << "        ";
	//print the data
		cout << a->data << " : ";
...
// check if null
	if(a==NULL)
	return;
// recursive call of its left subtree
	inorder(a->left);
// print data and country
	cout << a->data << "  ";
	cout << (*a).retrieve() << " || ";
// recursive call of its right subtree
	inorder(a->right);
// does the function until it is null (a is null)
}
